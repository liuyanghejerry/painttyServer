...
function Room(options) {
    ...
    room.router.reg('request', 'login', function(cli, obj) {
        var r_room = this;
        // name check
        ...
        if(r_room.options.password.length > 0){
            ...
            || obj['password'] != r_room.options.password){
                var ret = {
                    response: 'login',
                    result: false,
                    errcode: 302
        ...
        var ret = {
            ...
            info: {
                ...
                clientid: function(){
                    var hash = crypto.createHash('sha1');
                    hash.update(r_room.options.name 
                                + obj['name'] 
                                + r_room.options.salt
                                + (new Date()).getTime()
                                ,'utf8');
        ...
        return;
    }, room).reg('request', 'close', function(cli, obj) {
        var r_room = this;
        // check signed key
    ...
    }, room).reg('request', 'clearall', function(cli, obj) {
        var r_room = this;
        if(!obj['key'] || !_.isString(obj['key'])){
            var ret = {
                response: 'clearall',
    ...
    }, room).reg('request', 'onlinelist', function(cli, obj) {
        var r_room = this;
        if(!obj['clientid']){
            return;
        }
        ...
        var people = [];
        _.each(r_room.cmdSocket.clients, function(va) {
        ...
        var jsString = JSON.stringify(ret);
        r_room.cmdSocket.sendData(cli, new Buffer(jsString));
    }, room).reg('request', 'checkout', function(cli, obj) {
        var r_room = this;
        if(!obj['key'] || !_.isString(obj['key'])){
            var ret = {
                response: 'checkout',
